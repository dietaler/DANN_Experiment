...
def source_only(encoder, classifier, source_train_loader, target_train_loader, source_test_loader, target_test_loader):
    ...
    optimizer = optim.AdamW(
        ...
        lr=params.lr, weight_decay=1e-4  # L2 正則化
    )
...
def dann(encoder, classifier, discriminator, source_train_loader, target_train_loader, source_test_loader, target_test_loader):
    ...
    for epoch in range(params.epochs_DANN):
        ...
        for batch_idx, (source_data, target_data) in enumerate(zip(source_train_loader, target_train_loader)):
            ...
            # lambda_d = 0.05 + 0.5 * (epoch / params.epochs_DANN)  # 讓 domain loss 影響更平滑
            # total_loss = class_loss + lambda_d * domain_loss
            lambda_d = min(0.1 + 0.9 * (epoch / params.epochs_DANN), 0.5)
            total_loss = class_loss + lambda_d * domain_loss
